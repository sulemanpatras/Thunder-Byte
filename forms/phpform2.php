...
$sql2 = "SELECT services.category AS service_name, prices2.price
FROM services 
INNER JOIN prices2 ON services.id = prices2.id";
...
// $sql = "INSERT INTO `checkout2` (`name`, `email`, `phone`, `category`, `services`, `customer_wants`, `heard_about`) 
...
$pdf->lastPage();
ob_end_clean();
$pdfFileName2 = 'service_form_submission2.pdf';
$outputPath2 = rtrim($_SERVER['DOCUMENT_ROOT'], '/') . '/' . $pdfFileName2; // Absolute path to save the PDF file    
$pdf->Output($outputPath2, 'F');
//    readfile($outputPath);

// Read the PDF file content
$pdfContent = file_get_contents($outputPath2);


// Prepare and execute SQL query to insert PDF data into database
$stmt = $conn->prepare("INSERT INTO checkout2 (name, email, phone, category, services, customer_wants, heard_about, pdf_data) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
...
if ($stmt->execute()) {
    ...
    header('Location: thanks2.php?pdf2=' . $pdfFileName2);
    exit;
} 
...
<div class="container">
        ...
        <h2 class="mb-4">MERN Stack Service Form</h2>
